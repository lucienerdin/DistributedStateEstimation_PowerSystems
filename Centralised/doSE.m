function [V, converged, iterNum, z, z_est, error_sqrsum,errors] = doSE(baseMVA, bus, gen, branch, Ybus, Yf, Yt, V0, ref, pv, pq, measure, idx, sigma, real_values)
...
errors.Norm = NaN(max_it,1);
errors.Vm = NaN(max_it,size(bus,1));
errors.Va = NaN(max_it,size(bus,1));
...
%     genbus_row = findBusRowByIdx(bus, gbus);
    ...
    errors.Vm(i,:) = abs(Vm - real_values.Vm)';
    errors.Va(i,:) = abs(Va - real_values.Va ./180.*pi)';
end

errors.Norm = errors.Norm(~isnan(errors.Norm));
errors.Vm = errors.Vm(~any(isnan(errors.Vm), 2),:);
errors.Va = errors.Va(~any(isnan(errors.Va), 2),:);
iterNum = i;
